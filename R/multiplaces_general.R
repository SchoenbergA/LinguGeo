...
#' @export mergeMultiPlacesX
#' @aliases mergeMultiPlacesX
#' @examples
#' # load librarys
#' require(LinguGeo)
#' require(spatstat)
...
mergeMultiPlacesX <- function(df,pos_x,pos_y,col,col_ls){
  ...
  # clean up class column duplicates due to merging equal classes
  for(k in 1:nrow(df2)){
    d <- unlist(strsplit(as.character(df2[k,colnames(df2)[col+1]]), split=", "))
