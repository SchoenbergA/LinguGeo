...
#' @export mergeMultiPlaces3
#' @aliases mergeMultiPlaces3
#' @examples
#' # load librarys
#' require(LinguGeo)
#' require(spatstat)
...
mergeMultiPlaces3 <- function(df,pos_x,pos_y,col){
  ...
  for(n in 1:length(uc)){
    ...
    # check for dupbilcates
    if(df2$n[which(df2$places==uc[n])][1]>1) {

      # loop for all dublicates (2: n dublicates)
      for(i in 2:length(df2$places[which(df2$places==uc[n])])) {
        # raw code for column
        df2[,col+1][which(df2$places==uc[n])][1] <- paste0(df2[,col+1][which(df2$places==uc[n])][1],
                                                           ", ",
                                                           df2[,col+1][which(df2$places==uc[n])][i])
  ...
  for(k in 1:nrow(df2)){
    d <- unlist(strsplit(as.character(df2[k,3]), split=", "))
    df2$new[k] <-paste(unique(sort(d)), collapse = ', ')}# sort to avoid unequal order of elements
